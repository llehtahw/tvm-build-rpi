...
sed -i -e 's/USE_LIBBACKTRACE AUTO/USE_LIBBACKTRACE OFF/' -e 's/USE_VULKAN OFF/USE_VULKAN ON/' -e "s|USE_KHRONOS_SPIRV OFF|USE_KHRONOS_SPIRV ${PWD}/install.rpi|" -e 's/USE_CPP_RPC OFF/USE_CPP_RPC ON/' build.tvm.rpi/config.cmake
